...
class dgFtp
{
	...
	public function __call($name, $args)
	{
		...
		if ($func === 'ftp_connect' || $func === 'ftp_ssl_connect') {
			...
			throw new dgFtpException("Not connected to FTP server. Call connect() or ssl_connect() first.");
		...
		if (!$silent && $this->errorMsg !== NULL) {
			...
			throw new dgFtpException($this->errorMsg);
		}
	...
	public function mkDirRecursive($dir)
	{
		...
		while (!empty($parts)) {
			...
			try {
				...
				if (!$this->isDir($path)) {
					throw new dgFtpException("Cannot create directory '$path'.");
				}
			}
			$path .= '/';
		}
...
class dgFtpException extends Exception
{
}